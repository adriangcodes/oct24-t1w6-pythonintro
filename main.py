# print(2 + 3)

# # The below line prints Hello World
# # Python interpreter will ignore comments
# # Use command / to toggle comments
# print('Hello World!')

# # print('Goodbye') # See ya!

# # Data types

# # Numbers

# # int
# # 5
# # 1000
# print(type(5))

# # float
# # 2.5
# # 5.3
# # 3.14159
# print(type(3.14159))

# # complex
# # 2 + 3j
# print(type(2 + 3j))

# # Number Operations
# print(2 + 3)
# print(2 - 3)
# print(2 * 3) # Multiplication
# print(10 / 5) # Float division
# print(10 // 5) # Integer division
# print(2 // 3) # Integer division
# print(int(2 // 3)) # Integer division with typecast
# print(5 % 2) # Modulo - integer division giving remainder

# # Strings
# # "Hello World!"
# # 'Hello World!'
# print('Hello!')
# print("Hello!")
# print("It's Monday!")
# print('It\'s Monday!')
# print(type('Hello'))

# # Concatenation
# print('Hello ' + 'World')

# # String Methods
# print('Hello'.upper())
# print('Hello'.lower())
# print('Hello World'.replace('World', 'Monty Python'))

# # Boolean
# # True
# # False
# print(True)
# print(type(True))

# # Falsy values
# print(bool(0))
# print(bool(''))
# print(bool('asfda'))
# print(bool(None))
# print(bool([]))
# print(bool(()))
# print(bool({}))

# # Comparison Operators
# # <, >, <=, >=, ==
# print(2 < 3)
# print(2 > 3)
# print(5 <= 5)
# print(5 == 5)

# # Logical Operators
# # and, or, not
# print(True and False)
# print(True or False)
# print(5 > 3 and 2 < 5)

# Variables
num1 = 5
num2 = 4
print(num1 + num2)
num2 = 10 # New value assigned overrides previous variable, previous value is destroyed
print(num1 + num2)

# String interpolation
name = 'Adrian'
location = 'Sydney'
print('My name is ' + name + ' and I live in ' + location)

# f-string
print(f'My name is {name} and I live in {location}')
